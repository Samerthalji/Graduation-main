        ...
        <div
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              {applyError && (
                <div className="mb-4 bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-3 text-sm font-semibold">
                  ⚠️ {applyError}
                </div>
              )}
              <button type="submit" disabled={!cvFile || submitting}
                className={`w-full font-bold py-3.5 rounded-xl transition-all duration-200 shadow-lg ${cvFile && !submitting ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
                {submitting ? (
